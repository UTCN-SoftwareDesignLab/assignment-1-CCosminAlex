...
class ReviewServiceTest {
    ...
    void getReviewsForItem() {
        Users users = Users.builder().username(randomString()).password(randomString()).email(randomEmail()).build();
        userRepository.save(users);
        Item item = Item.builder().name(randomString()).description(randomString()).build();
        item = itemRepository.save(item);

        Review reviewToBeAdded = Review.builder().item(item).users(users).text(randomString()).build();
